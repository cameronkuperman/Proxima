...
CREATE INDEX idx_user_tutorials_user_id ON public.user_tutorials(user_id);
...
-- Create function to initialize tutorial record for new users
CREATE OR REPLACE FUNCTION initialize_user_tutorial()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.user_tutorials (user_id)
  VALUES (NEW.id)
...
-- Optional: Automatically create tutorial record when user signs up
-- CREATE TRIGGER create_user_tutorial_on_signup
-- AFTER INSERT ON auth.users
-- FOR EACH ROW EXECUTE FUNCTION initialize_user_tutorial();