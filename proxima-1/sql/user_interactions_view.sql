-- User Interactions View for Timeline
-- This creates a unified view of all user health interactions
-- Run this in Supabase SQL Editor

-- First, create indexes for performance (if they don't exist)
CREATE INDEX IF NOT EXISTS idx_quick_scans_user_created ON quick_scans(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_deep_dive_user_created ON deep_dive_sessions(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_photo_sessions_user_created ON photo_sessions(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_conversations_user_created ON conversations(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_medical_reports_user_created ON medical_reports(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_tracking_data_user_created ON tracking_data_points(user_id, created_at DESC);

-- Drop the view if it exists (for updates)
DROP VIEW IF EXISTS user_interactions;

-- Create the unified interactions view
...
-- Add comment
COMMENT ON VIEW user_interactions IS 'Unified view of all user health interactions for timeline display';